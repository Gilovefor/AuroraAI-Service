...
#ifdef CHATSERVER_RESOURCE_DIR
        const std::filesystem::path filePath = std::filesystem::path(CHATSERVER_RESOURCE_DIR) / "dist" / "assets" / fileName;
#else
        const std::filesystem::path filePath = std::filesystem::path("../AIApps/ChatServer/resource/dist/assets") / fileName;
#endif
