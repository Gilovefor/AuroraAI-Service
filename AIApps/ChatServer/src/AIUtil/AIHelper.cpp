...
/* 
统一管理AI相关的核心逻辑，包括：
1. 存储用户的历史对话消息（messages），并提供添加和恢复消息的方法
2. 构建聊天请求体，发送给AI模型，并解析响应
3. 支持不同AI模型的策略模式（AIStrategy），通过工厂模式（StrategyFactory）动态切换
4. 处理AI响应中的工具调用逻辑，结合AIConfig和AIToolRegistry实现二次调用
5. 提供一个通用的 request 方法，允许发送自定义请求体
*/
