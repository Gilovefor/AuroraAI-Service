/**
 * 开发 (npm run dev)：请求走同源 `/api/*`，由 Vite（默认 5173）代理到 `.env.development` 里的 `VITE_API_TARGET`。
 * 生产构建：默认无前缀，与 C++ 静态站同域。
 */
const apiBaseUrl = (import.meta.env.VITE_API_BASE_URL || '').replace(/\/$/, '')
const apiPrefix = import.meta.env.VITE_API_PREFIX ?? (import.meta.env.DEV ? '/api' : '')
